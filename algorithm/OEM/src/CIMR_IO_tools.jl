...
function get_data_for_band(fn, band, direction="forward";verbose=false)
    ncd = NCDataset(fn)
    d = ncd.group[band]
...
"""
function project_data(fn, target_band, direction="forward")
    gm = pr[].geometry #pyresample.geometry
    kdt = pr[].kd_tree #pyresample.kd_tree 
    data = (; (Symbol(b) => get_data_for_band(fn, b, direction) for b in bands)...)
    target_lat = data[Symbol(target_band)].lat
    target_lon = data[Symbol(target_band)].lon
    target_lon[target_lon .> 180] .-= 360
    println("min and max lon ", minimum(target_lon), maximum(target_lon))
    @show size(target_lat)
    @show size(target_lon)
